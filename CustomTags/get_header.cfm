...
<cfif isdefined("attributes.collection_id") AND len (#attributes.collection_id#) gt 0>
	<cfquery name="whatColl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
		...
		GROUP BY collection_cde, institution_acronym
	</cfquery>
...
<cfif len(#institution#) gt 0 and len(#collection#) gt 0>
	...
	<cfset hasHeader="UAMHerp,UAMMamm,MSBMamm,DGRMamm,KWPEnto,UAMEnto,UAMHerb,MCZBird,MCZMala,MCZAmph,MCZRept,MCZMamm,MCZBOM,MCZBangs,MCZEchT,MCZChon,MCZFish">
...
<cfset session.headerContent = "#header2#">
