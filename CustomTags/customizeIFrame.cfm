...
<cfoutput>
	<cfif isdefined("session.currentStyleSheet") and len(trim(session.currentStyleSheet)) gt 0>
		...
		<cfhtmlhead text='<link rel="custom:alternate stylesheet" type="text/css" href="/includes/css/#trim(session.currentStyleSheet)#.css" title="#trim(session.currentStyleSheet)#">'>
