...
<cffunction name="checkAgent" access="remote" returnformat="json">
    ...
    <cfif agent_type is "person">
        ...
        <cfif refind('[A-Z]{3,}',preferred_name) gt 0>
            <cfset problems=listappend(problems,'Check for abbreviations and acronyms (preferred name is short)',';')>
        </cfif>
        <cfif Compare(ucase(preferred_name), preferred_name) is 0 or Compare(lcase(preferred_name), preferred_name) is 0>
            <cfset problems=listappend(problems,'Check case of preferred name',';')>
        </cfif>
        <cfif preferred_name does not contain " ">
            <cfset problems=listappend(problems,'Do not create unnecessary variations of `unknown.` (preferred name is one word)',';')>
        </cfif>
        <cfif preferred_name contains ".">
            <cfset problems=listappend(problems,'Do not unnecessarily abbreviate (preferred_name contains period)',';')>
        </cfif>
...
<cfset gcaurl="#replace(application.serverRootURL,'https','http')#/component/utilities.cfc?method=georeferenceAddress&address=#URLEncodedFormat(thisAddress)#">
								<cfdump var=#gcaurl#>
