...
<cfquery name="getData" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
	select
		...
		get_formatted_taxname(flat.collection_object_id) SCI_NAME_WITH_AUTH,
		get_taxon(flat.collection_object_id,'SUBFAMILY') SUBFAMILY,
		get_taxon(flat.collection_object_id,'tribe') TRIBE,
