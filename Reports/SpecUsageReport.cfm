...
<cfoutput>
	...
	<cfif action is "buildIt">
		<cfset session.projectReportTable="projTable#left(session.sessionKey,10)#">
		<cftry>
		...
		<cfif len(publication_id) gt 0>
			...
			<cfloop query="p">
				<cfquery name="insPub" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
					insert into #session.projectReportTable# (
						report_title,
						publication_id,
