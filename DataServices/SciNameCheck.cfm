...
<cfif action is "nothing">
	<font size="12">Scientific Name Checker Tool</font>
	<br><br /><br><br /><strong>Tool Description:</strong>
	
	<br><br /><br><br />This tool is helpful to use prior to bulkloading object records. The report generated by the tool provides information about whether a taxon name is in Arctos or not. If it is not, you will need to add it prior to bulkloading your object records using that identification or modify your bulkload data to use a taxon that is included in Arctos.
	<br><br />This form considers only namestrings (that is, taxon_name.scientific_name) so will have a high false failure rate for data with complex names (Name sp., etc.). See <span class="http://handbook.arctosdb.org/documentation/bulkloader.html#taxonomy">bulkloader taxonomy documentation</span> and <span class="http://handbook.arctosdb.org/documentation/taxonomy#taxon-name>Taxonomy documentation</span> for the full scoop on taxonomy.

	<br><br /><br><br />Upon upload, Arctos will return data to you in a new .csv file which you can download and review. The file will contain the following columns of data:
		<ul>
	...
	<br><br /><br><br /><strong>Upoload File Requirements:</strong>
	
	<br><br /><br><br />The required file for this tool is a .csv with one column titled “scientific_name.” The resulting report will tell you which scientific names are in Arctos. Scientific names already in Arctos will return “name in Arctos.” Scientific names that are not in Arctos will return “FAIL” and may also return close matches that are in Arctos. This allows you to decide if you have potential misspellings in your bulkload data that can be corrected prior to bulkloading.
