	...
	<cfloop query="d">
		<cfif len(num) gt 0>
			...
			<cfif isThere.recordcount is 0>
				...
				<cfquery name="entby" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
				...
				<cfquery name="qrecfrom" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
					select agent_id from agent_name where agent_name='#thisAgent#' group by agent_id
				</cfquery>
				...
				<cfif len(recfrom2) gt 0>
					...
					<cfset thisAgent=trim(thisAgent)>
					<cfset thisAgent=replace(thisAgent,"  "," ","all")>
					<cfquery name="qrecfrom2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
						select agent_id from agent_name where agent_name='#thisAgent#' group by agent_id
					</cfquery>
