...
----->
	...
	<cfoutput>
		...
		<cfif isdefined("relationshiptype1") and len(relationshiptype1) gt 0>
			...
			<cfif len(relationship1) gt 0>
				...
				<cfelseif right(relationshiptype1,14) is "cataloged_item">
					...
					<cfset srch="#srch# AND upper(mr_cataloged_item1.guid) || upper(mr_cataloged_item1.cat_num) || upper(mr_cataloged_item1.scientific_name) like '%#ucase(escapeQuotes(relationship1))#%' ">
		...
		<cfif (isdefined("publication_id") and len(publication_id) gt 0)>
			<cfset tabls = "#tabls#,media_relations mr_publication">
			<cfset whr ="#whr# AND media_flat.media_id = mr_publication.media_id ">
			<cfset srch="#srch# AND mr_publication.media_relationship like '% publication' and mr_publication.related_primary_key = #publication_id#">
