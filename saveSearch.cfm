	...
	<table border>
		...
		<cfloop query="archive">
			<tr id="ar_#archive_name#">
				...
				<td>
					<cfif len(doi) gt 0>
						#doi# <a href="https://doi.org/#doi#">[ click ]</a>
					<cfelse>
...
<cfloop query="hasCanned">
	<tr id="tr#canned_id#">
		...
		<td>
			<span class="likeLink" onclick="window.open('/saveSearch.cfm?canned_id=#canned_id#','_mail','height=300,width=400,resizable,scrollbars')">Mail</span>
		</td>
	</tr>
</cfloop>
</table>
...
<cfif action is 'emailSearch'>
	...
	<form name="mailMe" method="post" action="saveSearch.cfm">
		<input type="hidden" name="action" value="preview_mail">
...
<cfif action is 'preview_mail'>
	<cfoutput>
		<cfif len(address) is 0>
			...
			<p>
				<form name="m" method="post" action="saveSearch.cfm">
