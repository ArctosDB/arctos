...
<cfif #action# is "nothing">
	...
	<ul>
		<li>
			...
			<br><a href="/findContainer.cfm?container_remark=No%20specimens%20with%20NK%20%20&container_type=cryovial">
				findContainer: cryovial with container remark contains "No specimens with NK"
			</a>
		</li>
...
<cfif #action# is "getBox">
	...
	<form name="findLoc" method="post" action="dgr_locator.cfm">
		...
		<cfquery name="f" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
...
<cfif #action# is "findLoc">
	...
	<form name="order" method="post" action="dgr_locator.cfm">
		...
		<cfquery name="locs" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
			select locator_id,
				freezer,
				rack,
				box,
			...
			<cfloop query="locs">
				...
				<cfquery name="thisRec" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
					...
					from
						...
						inner join flat ON (flat.collection_object_id = coll_obj_other_id_num.collection_object_id)
					WHERE
						other_id_type='NK'
...
<cfif #action# is "boxView">
	...
	<cfquery name="bView" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
...
<cfif #action# is "moveABox">
	<cfoutput>
		...
		<cfquery name="isItThere" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
		...
		<cfelse>
			...
			<cfquery name="contents" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
			...
			<cfloop query="contents">
				<!--- move the tissue to the new box if there is an NK and a tissue.
					...
					<cfif len(#NK#) gt 0 and len(#TISSUE_TYPE#) gt 0>
						<!--- spiffy --->
						<cfquery name="old" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
			...
			<!---
			<cfquery name="moveBox" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
				UPDATE dgr_locator
					set freezer=#freezer#,
					rack=#rack#,
					box=#box#
