    ...
    <cfoutput>
        <cftransaction>
            <cfloop list="#partID#" index="i">
                <cfquery name="dpa" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
                    delete from specimen_part_attribute where collection_object_id=#i#
                </cfquery>
                <cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
