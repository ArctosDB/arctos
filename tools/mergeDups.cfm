...
<a href="mergeDups.cfm?autorun=yep">Autorun</a>
 <p>First Hundred Duplicates:
	<cfquery name="findAllDups" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
		select 
...
<cfif #action# is "merge">
	...
	<cfset problems = "">
		...
		">
		<cfquery name="one" ddatasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
		...
		<cfif len(#problems#) is 0>
	<cftransaction>
		<cfquery name="goodID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
		...
		<cfloop query="goodID">
			<br>create ID			
			<cfquery name="newInsId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
			...
			<cfquery name="newInsIdTax" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
			...
			<cfquery name="IDAgnt" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
				select * from identification_agent where IDENTIFICATION_ID = #IDENTIFICATION_ID#
			</cfquery>
			<cfloop query="IDAgnt">
			<br>insert ID agent(s)
			<cfquery name="newInsIdAgnt" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
		...
		<cfquery name="badpart" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
			select container.container_id,container.parent_container_id from
				specimen_part,
				coll_object,
				coll_obj_cont_hist,
		...
		<cfquery name="mkenc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
