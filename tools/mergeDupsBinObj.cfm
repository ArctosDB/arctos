...
<a href="mergeDupsBinObj.cfm?autorun=yep">Autorun</a>
	...
	<cfoutput>
		...
		<cfloop query="findAllDups">
			...
			<tr>
				...
				<td>
					<cfif len(#dupRec.collection_object_id#) gt 0 and len(#dupRec.subject#) is 0>
						<a href="mergeDupsBinObj.cfm?action=merge&id1=#collection_object_id#&id2=#RELATED_COLL_OBJECT_ID#">Merge</a>
						<cfif #autorun# is "yep">
							<script>
								document.location = 'mergeDupsBinObj.cfm?autorun=yep&action=merge&id1=#collection_object_id#&id2=#RELATED_COLL_OBJECT_ID#';
							</script>
						</cfif>
					<cfelse>
						no merge
...
<cfif #action# is "merge">
	<cfoutput>
		...
		<cfquery name="cn1" datasource="#application.uam_dbo#">
			select cat_num from cataloged_item where collection_object_id=#id1#
		</cfquery>
		
		<cfquery name="cn2" datasource="#application.uam_dbo#">
			select cat_num from cataloged_item where collection_object_id=#id2#
		</cfquery>
		<cfquery name="moveIt" datasource="#application.uam_dbo#">
			update binary_object set derived_from_cat_item=#id2# where
			derived_from_cat_item=#id1#
		</cfquery>
		
		<cfquery name="recattemp" datasource="#application.uam_dbo#">
			update cataloged_item set cat_num=9999999 where
			collection_object_id=#id2#
		</cfquery>
		<!---
		
		---->
		<cfquery name="recattempthree" datasource="#application.uam_dbo#">
			update cataloged_item set cat_num=#cn2.cat_num# where
			collection_object_id=#id1#
		</cfquery>
		
		<cfquery name="recattemptoo" datasource="#application.uam_dbo#">
			update cataloged_item set cat_num=#cn1.cat_num# where
			collection_object_id=#id2#
		</cfquery>
		<cfquery name="check1" datasource="#application.uam_dbo#">
			select cat_num from cataloged_item where collection_object_id=#id1#
		</cfquery>
		
		<cfquery name="check2" datasource="#application.uam_dbo#">
			select cat_num from cataloged_item where collection_object_id=#id2#
		</cfquery>
		<cfif (#cn1.cat_num# neq #check2.cat_num#) OR (#cn2.cat_num# neq #check1.cat_num#)>
			catnums don't jive
...
<cfabort>
	...
	<cftransaction>
		...
		<br><a href="/SpecimenDetail.cfm?collection_object_id=#id1#">One Record</a>
			...
			<cfif #autorun# is "yep">
			<cflocation url="mergeDupsBinObj.cfm?autorun=yep">
		</cfif>	
	</cfoutput>
</cfif>
