...
<cfquery name="caller.getLoan" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
    SELECT
		trans_date,
	...
	FROM
		...
		preferred_agent_name sponsor_name
	WHERE
		...
		project_sponsor.agent_id = sponsor_name.agent_id (+) and				
