...
<cfif len(bottom_margin) is 0>
	<cfset bottom_margin = ".25">
</cfif>
<cfif len(left_margin) is 0>
	<cfset left_margin = ".25">
</cfif>
<cfif len(right_margin) is 0>
	<cfset right_margin = ".25">
</cfif>
<cfdocument
        format="pdf"
        pagetype="letter"
...
<table borders='1'>
	...
	<cfif isdefined('permit_num')>
		<cfset sql="
			...
			where
				permit_num = '#permit_num#' AND
				permit.issued_by_agent_id = issuedBy.agent_id AND
				permit.issued_to_agent_id = issuedTo.agent_id ">
		<cfquery name="permit" datasource="#Application.web_user#">
			#preservesinglequotes(sql)#
		</cfquery>
		<div id='ledgerOptions' style='display:none;'>
		If you did not search by accession and select MVZ Report w/Accession,<br/>
		then the accession number will *NOT* be displayed in the title.
		</div>
		<!--- here is where I add in the permit selection--->
		<cfloop query='permit'>
		<div id='permitOptions'>
			Permit Number: <input type='text' readonly='readonly' name='permitNum' size='15' id='permit_num' value='#permit_num#'/>
