...
<cfif #action# is "getBox">
	...
	<form name="findLoc" method="post" action="dgr_locator.cfm">
		...
		<cfquery name="f" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cfoutput>
	...
	<!--- this query is very slow, so use stored procedure
		...
		<cfstoredproc datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#" procedure="makeDGRFreezerPositions">
	...
	<cfquery name="makeFreezer" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
	...
	<cfloop from="1" to="#numSlots#" index="s">
		<cfquery name="ns" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cfif #action# is "findLoc">
	...
	<form name="order" method="post" action="dgr_locator.cfm">
		...
		<cfquery name="locs" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			select locator_id,
				freezer,
				rack,
				box,
...
<cfif #action# is "boxView">
	...
	<cfquery name="bView" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cfif #action# is "moveABox">
	<cfoutput>
		...
		<cfquery name="isItThere" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
		...
		<cfelse>
			...
			<cfquery name="contents" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			...
			<cfloop query="contents">
				<!--- move the tissue to the new box if there is an NK and a tissue. 
					...
					<cfif len(#NK#) gt 0 and len(#TISSUE_TYPE#) gt 0>
						<!--- spiffy --->
						<cfquery name="old" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			...
			<!---
			<cfquery name="moveBox" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				UPDATE dgr_locator
					set freezer=#freezer#,
					rack=#rack#,
					box=#box#
