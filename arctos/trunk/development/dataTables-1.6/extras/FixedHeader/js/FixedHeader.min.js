...
return}var c=this;var d={aoCache:[],oSides:{top:true,bottom:false,left:false,right:false},oZIndexes:{top:104,bottom:103,left:102,right:101},oMes:{iTableWidth:0,iTableHeight:0,iTableLeft:0,iTableRight:0,iTableTop:0,iTableBottom:0},nTable:null,bUseAbsPos:false};
...
return}var e=b.fnSettings();c.nTable=e.nTable;e.aoDrawCallback.push({fn:function(){FixedHeader.fnMeasure();
...
return{nNode:a,nWrapper:c,sType:f,sPosition:"",sTop:"",sLeft:"",fnClone:d}},_fnMeasure:function(){var d=this.fnGetSettings(),a=d.oMes,c=jQuery(d.nTable),b=c.offset();
a.iTableWidth=c.outerWidth();a.iTableHeight=c.outerHeight();a.iTableLeft=b.left;a.iTableTop=b.top;
a.iTableRight=a.iTableLeft+a.iTableWidth;a.iTableRight=FixedHeader.oDoc.iWidth-a.iTableLeft-a.iTableWidth;
a.iTableBottom=FixedHeader.oDoc.iHeight-a.iTableTop-a.iTableHeight},_fnUpdatePositions:function(){var c=this.fnGetSettings();
this._fnMeasure();for(var b=0,a=c.aoCache.length;b<a;b++){if(c.aoCache[b].sType=="fixedHeader"){this._fnScrollFixedHeader(c.aoCache[b])
...
var a=d.nNode;d.nWrapper.style.width=jQuery(c.nTable).outerWidth()+"px";while(a.childNodes.length>0){a.removeChild(a.childNodes[0])
}var b=jQuery("tfoot",c.nTable).clone(true)[0];a.appendChild(b);jQuery("tfoot:eq(0)>tr th",c.nTable).each(function(e){jQuery("tfoot:eq(0)>tr th:eq("+e+")",a).width(jQuery(this).width())
});jQuery("tfoot:eq(0)>tr td",c.nTable).each(function(e){jQuery("tfoot:eq(0)>tr th:eq("+e+")",a)[0].style.width(jQuery(this).width())
})},_fnCloneTLeft:function(e){var b=this.fnGetSettings();var a=e.nNode;var d=jQuery("tbody tr:eq(0) td",b.nTable).length;
while(a.childNodes.length>0){a.removeChild(a.childNodes[0])}a.appendChild(jQuery("thead",b.nTable).clone(true)[0]);
a.appendChild(jQuery("tbody",b.nTable).clone(true)[0]);a.appendChild(jQuery("tfoot",b.nTable).clone(true)[0]);
jQuery("thead tr th:gt(0)",a).remove();jQuery("tbody tr td:not(:nth-child("+d+"n-"+(d-1)+"))",a).remove();
jQuery("tfoot tr th:gt(0)",a).remove();var c=jQuery("thead tr th:eq(0)",b.nTable).outerWidth();
a.style.width=c+"px";e.nWrapper.style.width=c+"px"},_fnCloneTRight:function(e){var b=this.fnGetSettings();
var a=e.nNode;var d=jQuery("tbody tr:eq(0) td",b.nTable).length;while(a.childNodes.length>0){a.removeChild(a.childNodes[0])
}a.appendChild(jQuery("thead",b.nTable).clone(true)[0]);a.appendChild(jQuery("tbody",b.nTable).clone(true)[0]);
a.appendChild(jQuery("tfoot",b.nTable).clone(true)[0]);jQuery("thead tr th:not(:nth-child("+d+"n))",a).remove();
jQuery("tbody tr td:not(:nth-child("+d+"n))",a).remove();jQuery("tfoot tr th:not(:nth-child("+d+"n))",a).remove();
var c=jQuery("thead tr th:eq("+(d-1)+")",b.nTable).outerWidth();a.style.width=c+"px";
e.nWrapper.style.width=c+"px"}};FixedHeader.oWin={iScrollTop:0,iScrollRight:0,iScrollBottom:0,iScrollLeft:0,iHeight:0,iWidth:0};
