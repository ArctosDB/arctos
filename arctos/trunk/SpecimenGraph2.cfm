...
<form name="browse" action="SpecimenGraph2.cfm" method="post">
				<tr>
							...
							<tr>
								...
								<td>
									<select name="chartType" size="1">
										<option value="flash">Flash (best viewing)</option>
							...
							<tr>
								...
								<td>
									<select name="type" size="1">
										...
										<option value="horizontalbar">horizontalbarar</option>
	...
	<cfloop list="#graphThis#" index="item">
		...
		<cfset y="Specimens">
			<cfif listcontains("family,phylorder,genus",#item#)>
				<cfset basSelect = "SELECT count(flat.cat_num)y_data,
					getTaxa(flat.collection_object_id,'#item#') as x_data">
				<cfset basGroup = "GROUP BY getTaxa(flat.collection_object_id,'#item#')">
			<cfelse>
				<cfset basSelect = "SELECT count(flat.cat_num)y_data,
					decode(flat.#item#,
					NULL,'not recorded',
			...
			<cfset basFrom = " FROM flat">
			<cfset basJoin = "INNER JOIN cataloged_item ON (flat.collection_object_id =cataloged_item.collection_object_id)">
			<cfset basWhere = " WHERE flat.collection_object_id IS NOT NULL ">	
			...
			<cfquery name="getGraph" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,cfid)#">
			 ...
			 <cfset chartName = "#item##cfid##cftoken#">
			...
			<cfif #chartType# is "Flash">
				<cfset fileExt = "swf">
			<cfelse>
				<cfset fileExt = "#chartType#">
			</cfif>
			<cffile 
				action="WRITE"
				charset="ISO-8859-1"
				 nameconflict="overwrite" 
				file="/var/www/html/temp/#chartName#.#fileExt#" 
				output="#chartBuffer#"> 
<img src="/temp/#chartName#.#fileExt#" height=#chartHeight# width=#chartWidth#>
	</cfloop>
</cfoutput>
</cfif>
