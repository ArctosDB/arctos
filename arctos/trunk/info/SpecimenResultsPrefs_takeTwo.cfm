...
create table cf_spec_res_cols (
	...
	delete from cf_spec_res_cols where column_name='max_elev_in_m';
	delete from cf_spec_res_cols where column_name='min_elev_in_m';
	insert into cf_spec_res_cols (column_name,sql_element) values ('','round(max_elev_in_m)');	
	insert into cf_spec_res_cols (column_name,sql_element) values ('elev_in_m','round(min_elev_in_m)');	
	update cf_spec_res_cols set category='locality' where column_name ='_elev_in_m';
	delete from cf_spec_res_cols where column_name='elev_in_m';
	insert into cf_spec_res_cols (column_name,sql_element) values ('_elev_in_m','');	
	insert into cf_spec_res_cols (column_name,sql_element) values ('_original_elevation','');
	update cf_spec_res_cols set category='locality' where column_name ='_original_elevation';	
...
insert into cf_spec_res_cols (column_name,sql_element) values ('dec_long','#flatTableName#.dec_long');
	...
	create table temp as select 	replace(replace(attribute_type,' ','_'),'-','_') a,attribute_type b from attributes group by attribute_type order by attribute_type;
update temp set b = 'ConcatAttributeValue(#flatTableName#.collection_object_id,'''' || b || '''')';
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td rowspan="2" valign="top"><span style="font-size:.85em;font-style:italic;">Check fields to show them in your results and downloads. Uncheck to remove.</span></td>
...
<form name="setPrefs" method="post" action="SpecimenResultsPrefs_takeTwo.cfm">
