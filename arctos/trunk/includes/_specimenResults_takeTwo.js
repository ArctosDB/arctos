...
function removeItems() {
	...
	DWREngine._execute(_cfscriptLocation, null, 'removeItems',theList, success_removeItems);
}
function success_removeItems (result) {
	//alert(result);
	var o1=document.getElementById('orderBy1').value;
	var o2=document.getElementById('orderBy2').value;
	var dr=document.getElementById('displayrows').value;
	var o = o1 + ',' + o2;
	getSpecResultsData(1,dr,o,'ASC');
}
	...
	if (collection_object_id < 1) {
		...
		var theInnerHtml = '<table class="specResultTab"><tr>';
			...
			if (result[0].COLUMNLIST.indexOf('DEC_LAT')> -1) {
				theInnerHtml += '<th>Decimal&nbsp;Latitude</th>';
			}
			if (result[0].COLUMNLIST.indexOf('DEC_LONG')> -1) {
				theInnerHtml += '<th>Decimal&nbsp;Longitude</th>';
			}
		...
		for (i=0; i<result.length; ++i) {
			theInnerHtml += '<tr>';
				if (killrow == 1){
					theInnerHtml += '<td><input type="checkbox" onchange="toggleKillrow(' + "'";
				...
				*/
				...
				if (result[0].COLUMNLIST.indexOf('DEC_LAT')> -1) {
					theInnerHtml += '<td>' + result[i].DEC_LAT + '&nbsp;</td>';
				}
				if (result[0].COLUMNLIST.indexOf('DEC_LONG')> -1) {
					theInnerHtml += '<td>' + result[i].DEC_LONG + '&nbsp;</td>';
				}
				if (result[0].COLUMNLIST.indexOf('COORDINATEUNCERTAINTYINMETERS')> -1) {
