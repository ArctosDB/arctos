...
function confirmAddAllPartLoan(){var a=$("#part_name").val();if(1==confirm("Are you sure you want to add all found "+a+" to the loan?"))document.location="/Loan.cfm?action=addAllSrchResultLoanItems&transaction_id="+$("#transaction_id").val()+"&part_name="+encodeURIComponent(a);else return!1}
...
function displayMedia(a){$("div[id^='jsonmedia_']").each(function(){var a=$.parseJSON($("#"+this.id).html());if(0<a.ROWCOUNT){var c='<div class="shortThumb"><div class="thumb_spcr">&nbsp;</div>';for(i=0;i<a.ROWCOUNT;++i)"audio"==a.DATA.mimecat[i]&&"mp3"==a.DATA.media_uri[i].split(".").pop()?(c+='<div class="one_thumb">',c+="<audio controls>",c+='<source src="'+a.DATA.media_uri[i]+'" type="audio/mp3">',c+='<a href="/exit.cfm?target='+a.DATA.media_uri[i]+'" target="_blank">download</a>',c+="</audio> ",
c+='<br><a target="_blank" href="/media/'+a.DATA.media_id[i]+'">Media Detail</a></p></div>'):$.ajax({url:"/component/functions.cfc?method=getMediaPreview&preview_uri="+a.DATA.preview_uri[i]+"&media_type="+a.DATA.mimecat[i]+"&returnformat=json&queryformat=column",dataType:"json",async:!1,success:function(e){c+='<div class="one_thumb">';c+='<a href="/exit.cfm?target='+a.DATA.media_uri[i]+'" target="_blank">';c+='<img src="'+e+'" class="theThumb"></a>';c+="<p>"+a.DATA.mimecat[i]+" ("+a.DATA.mime_type[i]+
