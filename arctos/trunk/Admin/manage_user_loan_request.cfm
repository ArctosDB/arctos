...
<cfquery name="whoAreYou" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
	select 
		collection,
		contact_agent_id
	from 
...
<cfif #action# is "nothing">
	...
	<cfquery name="all" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		select 
			username,
			PROJECT_TITLE,
...
<cfoutput>
	...
	<cfquery name="userdata" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
	...
	You are managing loan requests for user #req_name#. This user has submitted the following information about themselves:
	 ...
	 <cfquery name="prevAuth" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
	...
	<cfelse>
		...
		<cfquery  name="gotSpecs" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
			select 
				institution_acronym,
...
<cfoutput>
	<cfquery name="getLoanId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		select  
			USER_LOAN_ID,
			 PROJECT_TITLE,
			 PROJECT_DESCRIPTION   
...
<cfif #action# is "goGiveThemAuth">
	...
	<cfset whatToDo=querynew("collid,yesno")>
		...
		<cfloop query="whatToDo">
			<cfquery name="coll" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
				select collection_cde,institution_acronym
				from collection
...
<cfif #action# is "sendEmailFinally">
	<cfoutput>
	<cfquery name="curr" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
