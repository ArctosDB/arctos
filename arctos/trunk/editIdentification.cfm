...
<cfquery name="getID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
	SELECT
	...
	<cftransaction>
		<cfloop from="1" to="#NUMBER_OF_IDS#" index="n">
			...
			<cfif thisAcceptedIdFg is 1>
				...
				<cfquery name="newAcceptedId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
					UPDATE identification SET ACCEPTED_ID_FG=1 where identification_id = #thisIdentificationId#
				</cfquery>
			</cfif>
			<cfif thisAcceptedIdFg is "DELETE">
					<cfquery name="deleteId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
						DELETE FROM identification_agent WHERE identification_id = #thisIdentificationId#
					</cfquery>
					<cfquery name="deleteTId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
			...
			<cfelse>
				<cfquery name="updateId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
				...
				<cfloop from="1" to="#thisNumIds#" index="nid">
					...
					<cfif #thisIdAgntId# is -1 and (thisIdId is not "DELETE" and thisIdId gte 0)>
						<!--- new identifier --->
						<cfquery name="updateIdA" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
