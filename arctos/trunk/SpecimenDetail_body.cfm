...
<cfif #cgi.HTTP_HOST# contains "berkeley.edu">
	...
	SELECT count(roi.page_id) as thecount FROM GREF_ROI_NG roi,
	GREF_ROI_VALUE_NG val WHERE roi.roi_value_ng_id = val.id and
	val.collection_object_id = #collection_object_id#</cfquery>
	<cfif grefcount.thecount gt 0>
		<cfquery name="gref" datasource="#Application.web_user#">
		SELECT roi.page_id, roi.publication_id FROM GREF_ROI_NG roi,
		GREF_ROI_VALUE_NG val WHERE roi.roi_value_ng_id = val.id and
		val.collection_object_id = #collection_object_id#</cfquery>
		<tr>
			<td colspan="2">
		...
		<tr>
			<td valign="top" align="right" nowrap="nowrap">
				<strong>Field Notebook Page:</strong>
			</td>
			<td>
				&nbsp;<a 
