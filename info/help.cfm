...
<cfif isdefined("showAsHtml") and len(#showAsHtml#) is 0>
...
<!----------------------------------------------------------------------------------------------------->
<cfif #content# is "collection">
<cfset title="Collection Help">
	You may choose to set a specific collection in your preferences. Doing so will:
	<ul>
		<li>Set a collection-specific header on some forms</li>
		<li>Limit your Specimen Search results to that collection</li>
		<li>Limit dropdowns to that collection's data in SpecimenSearch</li>
	</ul>
</cfif>
...
<cfif #content# is "CollStats">
	...
	<cfquery name="stat" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		SELECT 
			collection,
			descr,
	...
	<cfquery name="cnt" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		select count(*) as cnt from cataloged_item
	</cfquery>
	<cfquery name="numColl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfif #content# is "get_proj_name">
	...
	<cfquery name="projName" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfif #content# is "get_permit_number">
	...
	<cfquery name="perm" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		select 
...
<cfif #content# is "lat_long_ref_source">
	...
	Existing values for lat_long_ref_source:
	<cfloop query="perm">
		<cfset thisLLRef = replace(lat_long_ref_source,"'","\'","all")>
		<a href="javascript: void(0);" onClick="opener.document.getElementById('lat_long_ref_source').value='#thisLLRef#';
			self.close();"><div style="text-indent:-2em;padding-left:2em;">#lat_long_ref_source#</div></a>
	</cfloop>
</cfif>
...
<cfset title="State or Province Help">
	State accepts:
	<ul>
		<li>A state or province (Alaska, British Columbia,  Chukotka)</li>
		<li>NULL, which returns records for which no state is recorded</li>
		<li>A pipe-delimited list ( | ), interpreted as "OR" (Alaska | British Columbia)</li>
	</ul>
</cfif>
<!----------------------------------------------------------------------------------------------------->
...
<!-------------------------------                v              --------------------------------------->
<!-------------------------------                v              --------------------------------------->
<cfif #content# is "verity">
	<cfset title="Verity Searches">
	The Verity Search feature performs a substring search against an index of common data. Not all data are indexed,
	 therefore not all data are searched. Data are refreshed periodically.
	 <p>Searches are not case sensitive.</p> Examples:
	 <table border>
	 	<tr>
		...
		<tr>
			...
			<td>contains the phrase Kenai Peninsula</td>
		</tr>
		<tr>
			<td>Kenai, Peninsula</td>
			<td>contains Kenai OR Peninsula</td>
		</tr>
		<tr>
			<td>Kenai AND Peninsula</td>
			<td>contains Kenai AND Peninsula</td>
		</tr>
	 </table>
</cfif>
