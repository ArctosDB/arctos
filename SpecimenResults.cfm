...
<script type='text/javascript' language="javascript" src='/includes/SpecimenResults.min.js?v=3.445'></script>
			...
			<cfif len(transaction_id) gt 0>
				...
				<cfelse>
					...
					<cfif partsForLoan.recordcount gte 1>
						...
						use this form to add an item from all found cataloged records (not necessarily just the ones visible on this page)
						to your loan.
						<p>
							For all catalog records, add this:
						</p>
						...
						<select name="goWhere" id="goWhere" size="1" onchange="reporter('#session.SpecSrchTab#');">
							...
							<optgroup label="Print Stuff">
			                    ...
			                    <option value="/SpecimenResultsDownload.cfm?action=bulkloaderFormat">
									Download for Catalog Record Bulkloader
								</option>
