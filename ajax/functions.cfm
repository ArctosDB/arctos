...
<cffunction name="agent_lookup" returntype="Any">
	...
	<cftry>
		<cfquery name="aid" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			SELECT 
				agent_id,
				'#v_f#' v_f,
				'#i_f#' i_f,
...
<cffunction name="makePart" returntype="any">
	...
	<cftry>
		<cftransaction>
			<cfquery name="ccid" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				select sq_collection_object_id.nextval nv from dual
			</cfquery>
			<cfquery name="updateColl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			...
			<cfif len(barcode) gt 0>
				<cfquery name="np" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
					select container_id from coll_obj_cont_hist where collection_object_id=#ccid.nv#
				</cfquery>
				<cfquery name="pc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
					select container_id from container where barcode='#barcode#'
				</cfquery>
				<cfquery name="m2p" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				...
				<cfif len(new_container_type) gt 0>
					<cfquery name="uct" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="getTypes" returntype="query">
	...
	<cfloop list="#idList#" index="cid">
		<cfquery name="ts" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="getCollObjByPart" returntype="query">
	...
	<cfif #other_id_type# is "catalog_number">
		<cfquery name="coll_obj" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="addPartToContainer" returntype="String">
	...
	<cftry>
		<cftransaction>
			...
			<cfquery name="cont" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			...
			<cfquery name="newparent" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				UPDATE container SET container_type = '#new_container_type#' WHERE
			...
			<cfif len(#part_id2#) gt 0>
				<cfquery name="cont2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
					select container_id FROM coll_obj_cont_hist where collection_object_id=#part_id2#
				</cfquery>
				<cfquery name="moveIt2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
					UPDATE container SET parent_container_id = #isGoodParent.container_id# WHERE
...
<cffunction name="addPartToLoan" returntype="string">
	...
	<cftransaction>
		<cftry>
			<cfquery name="n" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				select sq_collection_object_id.nextval n from dual
			</cfquery>
			
			<cfquery name="meta" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			...
			<cfif #subsample# is 1>
			<cfquery name="parentData" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				SELECT 
					coll_obj_disposition, 
					condition,
					part_name,
			...
			<cfquery name="newCollObj" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				INSERT INTO coll_object (
					COLLECTION_OBJECT_ID,
					COLL_OBJECT_TYPE,
					ENTERED_PERSON_ID,
			...
			<cfquery name="newPart" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				INSERT INTO specimen_part (
					COLLECTION_OBJECT_ID
					,PART_NAME
		...
		<cfquery name="setDisp" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="removeItems" returntype="string">
	...
	<cfoutput>
	<cfquery name="remove" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<!--------------
	<cftry>
		<cfquery name="tieRef" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="clientResultColumnList" returntype="string">
	...
	<cfquery name ="upDb" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<!------------------------------------->
 <cffunction name="deleteIdentification" returntype="string">
	...
	<cftry>
		<cftransaction>
			<cfquery name="delIdTax" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				delete from identification_taxonomy where 
				identification_id=#identification_id#
			</cfquery>
			<cfquery name="delIdA" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				delete from identification_agent where 
				identification_id=#identification_id#
			</cfquery>	
			<cfquery name="delId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<!------------------------------------->
 <cffunction name="flippedAccepted" returntype="string">
	...
	<cfargument name="identification_id" type="numeric" required="yes">
	<cftry>
		<cfquery name="flipOld" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
			update identification set accepted_id_fg=0
			where
			collection_object_id=#collection_object_id#
		</cfquery>
		<cfquery name="newIdentifier" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="kill_canned_search" returntype="string">
	...
	<cftry>
		<cfquery name="res" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="updateLoanItemRemarks" returntype="query">
	...
	<cftry>
		<cftransaction>
			<cfquery name="upIns" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="del_remPartFromLoan" returntype="query">
	...
	<cftry>
		<cftransaction>
			<cfquery name="killPart" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="moveContainerLocation" returntype="string">
	...
	<cftry>
		...
		<cftransaction>
			<cfquery name="alterTime" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="updatePartDisposition" returntype="query">
	...
	<cftry>
		<cfquery name="upPartDisp" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="moveContainer" returntype="string">
	...
	<cfset thisDate = dateformat(now(),"dd-mmm-yyyy")>
	<CFTRY>
		<cfquery name="thisID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
		...
		<cfif #thisID.recordcount# is 0>
			...
			<cfif #thisID.recordcount# is 1>
				<cfquery name="update" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
		...
		<cfif len(#thisContainerId#) gt 0>
			<cfquery name="putItIn" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="getPreviousBox" returntype="query">
	...
	<cfif #box# is 1>
		<cfif #rack# is 1>
			...
			<cfelse>
				<cfset tf = #freezer# -1 >
				<cfquery name="pf" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
	...
	<cfquery name="newLoc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
	
	</cfquery>
	<cfquery name="v" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<cffunction name="getContacts" returntype="string">
	<cfquery name="contacts" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
		select 
...
<cffunction name="getCollInstFromCollId" returntype="string">
	...
	<cftry>
		<cfquery name="getCollId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
