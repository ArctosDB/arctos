<cfinclude template="/includes/_header.cfm">
	...
	<cfoutput>
		...
		<cfelse>
			<cfquery name="permitExp" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
				select 
					permit_id,
					EXP_DATE,
					PERMIT_NUM,
			...
			<cfloop query="permitExp">
				...
				<cfif len(#whine#) gt 0>
					<cfif len(#CONTACT_AGENT_ID#) gt 0>
						<cfquery name="email" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
							SELECT
								ADDRESS
							FROM
								electronic_address
