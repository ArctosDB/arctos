...
<cfoutput> 
	...
	<cfquery name="geolDet" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
    	select 
			*
		from 
			geology_attributes,
	...
	<cfquery name="ctDepthUnit" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#" cachedwithin="#createtimespan(0,0,60,0)#">
	...
	<cfloop query="getLL"> 
        <form name="latLong#i#" method="post" action="editLocality.cfm" onSubmit="return noenter();">
            ...
            <span style="margin:1em;display:inline-block;padding:1em;border:3px solid <cfif accepted_lat_long_fg is 1>green<cfelse>red</cfif>;">
			<table>
            	<tr> 		 
                	<td>
...
<cfoutput>
	<cfloop from="1" to="#number_of_determinations#" index="n">
		...
		<cfif #thisAttribute# is "delete">
			<cfquery name="deleteGeol" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
				delete from geology_attributes where geology_attribute_id=#thisID#
			</cfquery>
		<cfelse>
			<cfquery name="upGeol" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
				update 
					geology_attributes 
				set 
...
<cfoutput>
		<cfquery name="changeGeog" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfif action is "saveLocalityEdit">
	...
	<cfquery name="edLoc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfoutput>
	<cfdump var=#form#>
	<cfquery name="isColl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
	...
	<cftransaction>
		<cfquery name="deleLatLong" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
			delete from lat_long where locality_id=#locality_id#
		</cfquery>
		<cftransaction action="commit">
		<cfquery name="deleLocality" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfif #action# is "clone">
	<cfoutput>
		<cftransaction>
			<cfquery name="nLocId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
				select sq_locality_id.nextval nv from dual
			</cfquery>
			<cfset lid=nLocId.nv>
			<cfquery name="oldLoc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
			...
			<cfif isdefined("keepAcc") and keepAcc is 1>
				<cfquery name="accCoord" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
			...
			<cfif isdefined("keepUnacc") and keepUnacc is 1>
				<cfquery name="uaccCoord" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfif ACCEPTED_LAT_LONG_FG is 1>
	<cfquery name="flagAllZero" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfquery name="upLatLong" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
	#preservesinglequotes(sql)#
</cfquery>
</cftransaction>
<cfquery name="getAcc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
...
<cfoutput>	
	<cfquery name="notAcc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		UPDATE lat_long SET accepted_lat_long_fg = 0 where
		locality_id=#locality_id#
	</cfquery>	
	<cfset sql = "
	...
	VALUES (
		...
		<cfset sql="#sql# )">
	<cfquery name="newLatLong" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		#preservesinglequotes(sql)#
	</cfquery>
	<cfquery name="getAcc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		select lat_long_id from lat_long where locality_id=#locality_id#
...
<cfif #action# is "deleteLatLong">
	<cfoutput>
		...
		<cfquery name="killLatLong" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
