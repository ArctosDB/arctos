...
function initializeMap(){$("#spresmapdiv").html("");new google.maps.InfoWindow;var a={zoom:3,center:new google.maps.LatLng(55,-135),mapTypeId:google.maps.MapTypeId.ROADMAP,panControl:!1,scaleControl:!0};map=new google.maps.Map(document.getElementById("spresmapdiv"),a);a=$("#cfgml").val();if(0==a.length)return!1;for(var a=a.split(";"),b=0;b<a.length;b++)createMarker(a[b]);a=new google.maps.LatLngBounds;for(b=0;b<markers.length;b++)a.extend(markers[b].getPosition());if(a.getNorthEast().equals(a.getSouthWest())){var b=
new google.maps.LatLng(a.getNorthEast().lat()+.05,a.getNorthEast().lng()+.05),c=new google.maps.LatLng(a.getNorthEast().lat()-.05,a.getNorthEast().lng()-.05);a.extend(b);a.extend(c)}map.fitBounds(a)}
function createMarker(a){var b=a.split(",");a=b[0];var c=b[1],e=b[2],b=b[3],d=new google.maps.LatLng(c,e),f={strokeColor:"#FF0000",strokeOpacity:.8,strokeWeight:2,fillColor:"#FF0000",fillOpacity:.15,map:map,center:d,radius:parseInt(b),zIndex:-99},c=a+" specimens; Error(m)="+b+'<br><span class="likeLink" onclick="addCoordinates(\''+c+","+e+"');\">add point to search</span>";crcl=new google.maps.Circle(f);var g=new google.maps.Marker({position:d,map:map,title:a+" specimens; Error(m)="+b,contentString:c,
zIndex:10});markers.push(g);var h=new google.maps.InfoWindow({content:c});google.maps.event.addListener(g,"click",function(){h.open(map,g)})}function confirmAddAllDL(){if(1==confirm("Are you sure you want to add all these specimens to the data loan?"))document.location="/Loan.cfm?action=addAllDataLoanItems&transaction_id="+$("#transaction_id").val();else return!1}
function confirmAddAllPartLoan(){var a=$("#loan_all_part_name").val();if(1==confirm("Are you sure you want to add all found "+a+" to the loan?"))document.location="/Loan.cfm?action=addAllSrchResultLoanItems&transaction_id="+$("#transaction_id").val()+"&part_name="+encodeURIComponent(a);else return!1}
function confirmAddAllPartLoanSS(){var a=$("#loan_all_part_name").val();if(1==confirm("Are you sure you want create subsample of all found "+a+" and add them to the loan?"))document.location="/Loan.cfm?action=SS_addAllSrchResultLoanItems&transaction_id="+$("#transaction_id").val()+"&part_name="+encodeURIComponent(a);else return!1}
...
function pickedTool(){var a;a=$("#usertools").val();"BerkeleyMapper"==a?openWindow("/bnhmMaps/bnhmMapData.cfm?"+$("#mapURL").val()):"BerkeleyMapperRM"==a?openWindow("/bnhmMaps/bnhmMapData.cfm?showRangeMaps=true&"+$("#mapURL").val()):"google"==a?openWindow("/bnhmMaps/kml.cfm"):"customize"==a?openCustomize():"removeRows"==a?removeRows():"saveSearch"==a?saveSearch($("#ServerRootUrl").val()+"/SpecimenResults.cfm?"+$("#mapURL").val()):"archiveSpecimens"==a?archiveSpecimens():"download"==a&&openWindow("/SpecimenResultsDownload.cfm?tableName="+
...
"position",["center","center"])})});function closeCustom(){var a="/SpecimenResults.cfm?"+$("#mapURL").val();window.location=a}function getPostLoadJunk(){var a=[];$("div[id^='CatItem_']").each(function(){var b=this.id.split("_")[1];a.push(b)});var b=a.toString();insertMedia(b);insertTypes(b);injectLoanPick();displayMedia()}
function displayMedia(a){$("div[id^='jsonmedia_']").each(function(){var a=$.parseJSON($("#"+this.id).html());if(0<a.ROWCOUNT){var c='<div class="shortThumb"><div class="thumb_spcr">&nbsp;</div>';for(i=0;i<a.ROWCOUNT;++i)"audio"==a.DATA.mimecat[i]&&"mp3"==a.DATA.media_uri[i].split(".").pop()?(c+='<div class="one_thumb">',c+="<audio controls>",c+='<source src="'+a.DATA.media_uri[i]+'" type="audio/mp3">',c+='<a href="/media/'+a.DATA.media_id[i]+'?open" target="_blank">download</a>',c+="</audio> ",c+=
'<br><a target="_blank" href="/media/'+a.DATA.media_id[i]+'">Media Detail</a></p></div>'):$.ajax({url:"/component/functions.cfc?method=getMediaPreview&preview_uri="+a.DATA.preview_uri[i]+"&media_type="+a.DATA.mimecat[i]+"&returnformat=json&queryformat=column",dataType:"json",async:!1,success:function(e){c+='<div class="one_thumb">';c+='<a href="/media/'+a.DATA.media_id[i]+'?open" target="_blank">';c+='<img src="'+e+'" class="theThumb"></a>';c+="<p>"+a.DATA.mimecat[i]+" ("+a.DATA.mime_type[i]+")";
c+='<br><a target="_blank" href="/media/'+a.DATA.media_id[i]+'">Media Detail</a></p></div>'}});c+='<div class="thumb_spcr">&nbsp;</div></div>';$("#"+this.id).html(c)}else $("#"+this.id).html("")})}
function insertMedia(a){var b=document.createElement("DIV");b.id="ajaxStatus";b.className="ajaxStatus";b.innerHTML="Checking for Media...";document.body.appendChild(b);jQuery.getJSON("/component/SpecimenResults.cfc",{method:"getMedia",idList:a,returnformat:"json",queryformat:"column"},function(a){try{var b=document.getElementById("ajaxStatus");b.innerHTML="Processing Media....";for(i=0;i<a.ROWCOUNT;++i){var d,f=a.DATA.COLLECTION_OBJECT_ID[i],g=a.DATA.MEDIA_ID[i],h=a.DATA.MEDIA_RELATIONSHIP[i];"cataloged_item"==
h?d="CatItem_"+f:"collecting_event"==h&&(d="SpecLocality_"+f);if(0<d.length){var l=document.getElementById(d),k='<a href="/MediaSearch.cfm?action=search&media_id='+g+'" class="mediaLink" target="_blank" id="mediaSpan_'+f+'">',k=k+"Media",k=k+"</a>";l.innerHTML+=k}}document.body.removeChild(b)}catch(m){b=document.getElementById("ajaxStatus"),document.body.removeChild(b)}})}
function insertTypes(a){var b=document.createElement("DIV");b.id="ajaxStatus";b.className="ajaxStatus";b.innerHTML="Checking for Types...";document.body.appendChild(b);jQuery.getJSON("/component/SpecimenResults.cfc",{method:"getTypes",idList:a,returnformat:"json",queryformat:"column"},function(a){var b=document.getElementById("ajaxStatus");try{for(b.innerHTML="Processing Types....",i=0;i<a.ROWCOUNT;++i){var d=a.DATA.TYPELIST[i],f="CatItem_"+a.DATA.COLLECTION_OBJECT_ID[i];if(0<f.length){var g=document.getElementById(f);
g.innerHTML+='<div class="showType">'+d+"</div>"}}}catch(h){}document.body.removeChild(b)})}
function injectLoanPick(){var a=$("#transaction_id").val();a&&($("body").append('<div id="ajaxStatus" class="ajaxStatus">Feching Loan Pick...</div>'),jQuery.getJSON("/component/SpecimenResults.cfc",{method:"getLoanPartResults",transaction_id:a,returnformat:"json",queryformat:"column"},function(a){$("div[id^='CatItem_']").each(function(){var c=this.id.split(/_(.+)?/)[1],e=!1,d='<table border width="100%">';for(i=0;i<a.ROWCOUNT;++i)a.DATA.COLLECTION_OBJECT_ID[i]==c&&(e=!0,d+='<tr><td nowrap="nowrap" class="specResultPartCell"><i>'+
a.DATA.PART_NAME[i],0<a.DATA.SAMPLED_FROM_OBJ_ID[i]&&(d+="&nbsp;sample"),d+="&nbsp;("+a.DATA.COLL_OBJ_DISPOSITION[i]+")</i> ["+a.DATA.BARCODE[i]+"]",d+='</td><td nowrap="nowrap" class="specResultPartCell">',d+='Remark:&nbsp;<input type="text" name="item_remark" size="10" id="item_remark_'+a.DATA.PARTID[i]+'">',d+='</td><td nowrap="nowrap" class="specResultPartCell">',d+='Instr.:&nbsp;<input type="text" name="item_instructions" size="10" id="item_instructions_'+a.DATA.PARTID[i]+'">',d+='</td><td nowrap="nowrap" class="specResultPartCell">',
d+='Subsample?:&nbsp;<input type="checkbox" name="subsample" id="subsample_'+a.DATA.PARTID[i]+'">',d+='</td><td nowrap="nowrap" class="specResultPartCell">',d+='<input type="button" id="theButton_'+a.DATA.PARTID[i]+'"',d+=' class="insBtn"',0<a.DATA.TRANSACTION_ID[i]?d+=' onclick="" value="In Loan">':(d+=' value="Add" onclick="addPartToLoan(',d+=a.DATA.PARTID[i]+');">'),null!==a.DATA.ENCUMBRANCE_ACTION[i]&&(d+="<br><i>Encumbrances:&nbsp;"+a.DATA.ENCUMBRANCE_ACTION[i]+"</i>"),d+="</td></tr>");e&&(d+=
"</table>",$("#CatItem_"+c).append(d))});$("#ajaxStatus").remove()}))}
function addPartToLoan(a){var b="item_instructions_"+a,c="subsample_"+a,e=document.getElementById("item_remark_"+a).value,b=document.getElementById(b).value,c=document.getElementById(c).checked,c=1==c?1:0,d=document.getElementById("transaction_id").value;jQuery.getJSON("/component/functions.cfc",{method:"addPartToLoan",transaction_id:d,partID:a,remark:e,instructions:b,subsample:c,returnformat:"json",queryformat:"column"},function(a){a=a.split("|");1==a[0]?(a=document.getElementById("theButton_"+a[1]),
a.value="In Loan",a.onclick=""):alert("An error occured!\n"+a[1])})}function reporter(a){var b=document.getElementById("goWhere").value;if(0==b.length)return alert("Pick a tool, then click the button."),!1;var c=b,e="?";0<b.indexOf("?")&&(e="&");c=c+e+("&table_name="+a);c+="&sort=guid";$("#goWhere")[0].selectedIndex=0;window.open(c)}
