...
function formatJSONLocality(){$('div[id^="jsonlocality_"]').length&&($("div[id^='jsonlocality_']").each(function(){var a=$.parseJSON($("#"+this.id).html());a=JSON.stringify(a,null,2);$("#"+this.id).html("<pre>"+a+"</pre>")}),$("#ssControl").append('<input type="button" id="ldcb" value="Expand JSON Locality" class="clrBtn" onclick="expLocDet()">'))}
...
function displayMedia(a){$("div[id^='jsonmedia_']").each(function(){console.log("i am doing media stuff");var a=$.parseJSON($("#"+this.id).html());if(0<a.length){console.log("got something");console.log(a);var c='<div class="shortThumb"><div class="thumb_spcr">&nbsp;</div>'}jQuery.each(a,function(a,b){console.log(b.MEDIA_ID);"audio"==b.MIMECAT&&"mp3"==b.MEDIA_URI.split(".").pop()?(c+='<div class="one_thumb">',c+="<audio controls>",c+='<source src="'+b.MEDIA_ID+'" type="audio/mp3">',c+='<a href="/media/'+
b.MEDIA_ID+'?open" target="_blank">download</a>',c+="</audio> ",c+='<br><a target="_blank" href="/media/'+b.MEDIA_ID[i]+'">Media Detail</a></p></div>'):$.ajax({url:"/component/functions.cfc?method=getMediaPreview&preview_uri="+b.PREVIEW_URI+"&media_type="+b.MIMECAT+"&returnformat=json&queryformat=column",dataType:"json",async:!1,success:function(a){c+='<div class="one_thumb">';c+='<a href="/media/'+b.MEDIA_ID+'?open" target="_blank">';c+='<img src="'+a+'" class="theThumb"></a>';c+="<p>"+b.MIMECAT+
" ("+b.MIME_TYPE+")";c+='<br><a target="_blank" href="/media/'+b.MEDIA_ID+'">Media Detail</a></p></div>'}})});0<a.length?(c+='<div class="thumb_spcr">&nbsp;</div></div>',$("#"+this.id).html(c)):$("#"+this.id).html("")})}
