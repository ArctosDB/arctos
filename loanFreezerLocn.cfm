...
<cfquery name="allCatItemsRaw" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#" cachedwithin="#createtimespan(0,0,60,0)#">
	#preservesinglequotes(sql)#
</cfquery>
...
<cfset a=1>
<cfset fileName = "FreezerLocation_#left(session.sessionid,10)#.csv">
...
<cfloop query="allCatItems">
	<cfquery name="freezer" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
		select 
			CONTAINER_ID,
			PARENT_CONTAINER_ID,
			CONTAINER_TYPE,
