...
<cfquery name="allCatItemsRaw" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#" cachedwithin="#createtimespan(0,0,60,0)#">
	#preservesinglequotes(sql)#
</cfquery>
...
<cfset a=1>
<cfset fileName = "FreezerLocation_#left(jsessionid,10)#.csv">
...
<cfloop query="allCatItems">
	<cfquery name="freezer" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,jsessionid)#">
		select 
			CONTAINER_ID,
			PARENT_CONTAINER_ID,
			CONTAINER_TYPE,
