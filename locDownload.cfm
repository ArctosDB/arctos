 ...
 <cfquery name="allRecords" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
...
<!--- loop through the records and append a line for each one --->
 ...
 <cfloop query="allRecords">
 	...
 	<cfquery name="thisRecord" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
		select 
		CONTAINER_ID,
		PARENT_CONTAINER_ID,
		CONTAINER_TYPE,
		...
		<cfloop query="thisRecord">
			...
			<tr>
				...
				---->
					...
					<cfquery name="p1" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
						select 
						CONTAINER_ID,
						PARENT_CONTAINER_ID,
						CONTAINER_TYPE,
				...
				<cfif #p1.label# gt 0>
					...
					<cfif len(#p1id#) gt 0>
						<cfquery name="p2" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
								select 
								CONTAINER_ID,
								PARENT_CONTAINER_ID,
								CONTAINER_TYPE,
				...
				<td><cfif len(#p2id#) gt 0>
					<cfquery name="p3" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
								select 
								CONTAINER_ID,
								PARENT_CONTAINER_ID,
								CONTAINER_TYPE,
					...
					<cfif len(#p3id#) gt 0>
						<cfquery name="p4" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
								select 
								CONTAINER_ID,
								PARENT_CONTAINER_ID,
								CONTAINER_TYPE,
						...
						<cfif len(#p4id#) gt 0>
						<cfquery name="p5" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
								select 
								CONTAINER_ID,
								PARENT_CONTAINER_ID,
								CONTAINER_TYPE,
						...
						<cfif len(#p5id#) gt 0>
						<cfquery name="p6" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionid)#">
								select 
								CONTAINER_ID,
								PARENT_CONTAINER_ID,
								CONTAINER_TYPE,
